...
public class InfGameBoard {
	...
	public static int[][] changeBoard(int[][] board, InfClientMessage message,
			int fmX, int fmY) {
	...
	public static InfClientMessage validateFirstMove(int[][] board,
			InfClientMessage message) {
	...
	public static InfClientMessage validateSecondMove(int[][] board,
			InfClientMessage message, int fmX, int fmY, String id) {
		...
		if (message.getMoveType().equals("server")) {
			if (board[message.getXcoord()][message.getYcoord()] == 0) {
				if (isFieldNearBy5x5(message, fmX, fmY)) {
					if (isFieldNearBy5x5(message, fmX, fmY) == true
							& isFieldNearBy3x3(message, fmX, fmY) == false) {
	...
	private static boolean isFieldNearBy3x3(InfClientMessage message, int fmX,
			int fmY) {
		int fmXcoord = fmX;
		int fmYcoord = fmY;
	...
	private static boolean isFieldNearBy5x5(InfClientMessage message, int fmX,
			int fmY) {
		int fmXcoord = fmX;
		int fmYcoord = fmY;
		...
		if ((curXcoord == fmXcoord || curXcoord == fmXcoord - 2
				|| curXcoord == fmXcoord + 2 || curXcoord == fmXcoord - 1 || curXcoord == fmXcoord + 1)
				& (curYcoord == fmYcoord || curYcoord == fmYcoord - 2
						|| curYcoord == fmYcoord + 2
						|| curYcoord == fmYcoord - 1 || curYcoord == fmYcoord + 1)) {
			return true;
		} else {
			return false;
		}
	...
	private static int[][] infectEnemies(int[][] board, InfClientMessage message) {
		...
		if (message.getMoveType().equals("server")) {
			...
			for (int i = 0; i <= 7; i++) {
				for (int j = 0; j <= 7; j++) {
					if (((i >= xcoord - 1) & (i <= xcoord + 1))
							& ((j >= ycoord - 1) & (j <= ycoord + 1))) {
						if (board[i][j] == 2) {
							board[i][j] = 1;
						}
	...
	private static int[][] lightFields(int[][] board, InfClientMessage message) {
		...
		for (int i = 0; i <= 7; i++) {
			for (int j = 0; j <= 7; j++) {
				if (((i >= xcoord - 2) & (i <= xcoord + 2))
						& ((j >= ycoord - 2) & (j <= ycoord + 2))) {
					if (board[i][j] == 0) {
						board[i][j] = 5;
					}
	...
	private static int[][] cleandLightFields(int[][] board,
			InfClientMessage message) {
		for (int i = 0; i <= 7; i++) {
			for (int j = 0; j <= 7; j++) {
				if (board[i][j] == 5) {
